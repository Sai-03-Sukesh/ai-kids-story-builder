  ...
  const GenerateStory=async()=>{
    ...
    //Generate AI story
      try{
        ...
        const story = JSON.parse(result?.response?.text() || '{}'); //Here .replace function is used

        //Generate image
        const imageResp = await axios.post('/api/generate-image', {
          prompt: 'Add text with title ' + story?.story_title +
                  " in bold text for book cover, " + 
                  story?.cover_image_prompt?.description+" with image style as "+story?.cover_image_prompt?.style
        });
  ...
  return (
    <div className='p-10 md:px-20 lg:px-40'> 
      ...
      <div className='flex justify-end my-10 flex-col items-end'>
        <Button color='primary' disabled={loading} className='p-10 text-2xl'
